...
"POLG_HCVJF_Qi_2014"]
dms_done = os.listdir("../dataset/ProteinGym/result/esm-variant/")
dms_done = [i.split(".")[0] for i in dms_done]
print(dms_done)
for i in tqdm(range(len(sequence_list))):
    ...
    if (~sequence_list.iloc[i]["includes_multiple_mutants"]) and (DMS_id not in dms_id_ignored) and (DMS_id not in dms_done):
        ...
        with torch.no_grad():
            ...
            del batch_tokens_masked, all_token_probs
            ...
            for i in range(len(mutation_seqs)):
                ...
                masked_marginal_score = masked_token_probs[0, idx, mt_encoded] - masked_token_probs[0,idx,wt_encoded] #masked-marginals
        ...
        df_mutation_score = pd.DataFrame({"mutation":mutation_seqs["mutant"],"score":mutation_seqs["DMS_score"],"wt_marginals":wt_marginals_list,"masked_marginals":masked_marginals_list})
        df_mutation_score.to_csv("../dataset/ProteinGym/result/esm-variant/"+DMS_id+".csv",index=False)
        ...
        print(wt_marginals_spearman,masked_marginals_spearman,seq_len)

        spearman_list.append([DMS_id,wt_marginals_spearman,masked_marginals_spearman,seq_len])
