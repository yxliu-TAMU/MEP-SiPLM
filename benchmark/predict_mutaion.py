...
def load_args():
    ...
    parser.add_argument(
        "--datapath", type=str, default="./datasets/dms7", help="dataset prefix"
    )
    parser.add_argument(
        "--outdir", type=str, default="./logs_pst/dms7", help="output directory",
    )
    parser.add_argument(
...
def main():
    ...
    for i, protein_id in zip(cfg.protein_id, protein_ids):
        print("-" * 40)
        print(f"Protein id: {protein_id}, i= {i}")
        mutations = mutations_list[i]
        graph, protein_dict = dataset[i]
        print(f"protein_dict{protein_dict['protein']['ID']}")
        df = mutations.copy()
